...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (type === 'created') {
        // Queries created by operations team
        filter = {
          $or: [
            { team: 'operations' },
        ...
        // All queries visible to operations team
        filter = {
          $or: [
            { team: 'operations' },
            { markedForTeam: 'operations' },
  ...
  try {
    ...
    for (let i = 0; i < queryTexts.length; i++) {
      ...
      const newQuery = {
        ...
        branch: branch || applicationDetails?.branchName || 'Unknown Branch',
        branchCode: branchCode || applicationDetails?.branchCode || '',
        queries: [{
          id: `${baseId}-query-${i}`,
          text: queryText,
          timestamp: new Date().toISOString(),
        ...
        remarks: [],
        applicationBranch: applicationDetails?.branchName || branch || '',
        applicationBranchCode: applicationDetails?.branchCode || branchCode || ''
      };
