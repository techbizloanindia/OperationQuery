    ...
    if (missingFields.length > 0) {
      ...
      return NextResponse.json(
        { 
          ...
          optionalColumns: ['AppDate', 'LoanNo', 'Email', 'Product', 'TaskName', 'Login', 'ReceivedOn', 'Location', 'Asset Type', 'App Status', 'Sanction Date']
        },
        { status: 400 }
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        const sanctionedApplicationData: CreateSanctionedApplicationData = {
          ...
          approvedBy: login || undefined,
          remarks: `Imported from CSV${appDate ? ` - App Date: ${appDate}` : ''}${loanNo ? `, Loan No: ${loanNo}` : ''}${product ? `, Product: ${product}` : ''}${taskName ? `, Task: ${taskName}` : ''}${receivedOn ? `, Received: ${receivedOn}` : ''}${location ? `, Location: ${location}` : ''}${assetType ? `, Asset Type: ${assetType}` : ''}${appStatus ? `, Status: ${appStatus}` : ''}`,
          conditions: [],
