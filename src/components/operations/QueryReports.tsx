  ...
  allQueries.forEach(queryGroup => {
    if (queryGroup && Array.isArray(queryGroup.queries)) {
      queryGroup.queries.forEach((query: any, index: number) => {
        ...
        // Safely handle query ID which might be undefined
        let queryIdNum = index;
        if (query.id) {
          // If id exists, try to parse it as before
          const idParts = query.id.split('-');
          if (idParts.length > 0) {
            queryIdNum = parseInt(idParts[0]) + index;
          }
        }
        
        individual.push({
          ...queryGroup,
          id: queryIdNum, // Safe ID handling
...
export default function QueryReports() {
  ...
  const { data: allQueries, isLoading, isError, error, refetch } = useQuery({
    ...
    refetchInterval: 3 * 1000, // Every 3 seconds for real-time updates
  ...
  useEffect(() => {
    const unsubscribe = queryUpdateService.subscribe('operations', (update) => {
      ...
      // Always refresh reports when any query is updated
      if (update.action === 'approved' || update.action === 'created' || update.action === 'updated') {
        console.log(`📈 Updating reports with ${update.action} query: ${update.appNo}`);
        if (update.resolvedBy) {
          console.log(`👤 Approved by: ${update.resolvedBy}`);
        }
        
        // Immediately refresh the reports to show the updated data
        refetch();
      }
    });
  ...
  const downloadReport = async (format: 'csv' | 'excel') => {
    try {
      ...
      console.log('🔄 Refreshing data before export...');
      await refetch();
      ...
      console.log(`📊 Exporting ${dataToExport.length} records with latest status updates...`);
  ...
  <Table>
   ...
   ${csvData.map(row => 
     `<Row>${row.map((cell, index) => {
      ...
      showNotification('success', `✅ Successfully exported ${recordCount} records to ${format.toUpperCase()} format at ${exportTime}`);
      console.log('📊 Data includes latest status updates from Approval Dashboard');
      
    } catch (error) {
      console.error('Export failed:', error);
      showNotification('error', `Failed to export ${format.toUpperCase()} file. Please try again.`);
    }
  };
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                title="Refresh data to get latest status updates"
              >
                🔄 Refresh Data
              </button>
              <button
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredData.length === 0 ? (
                    ...
                    filteredData.map((query, queryMapIndex) => {
                      const dateTime = formatDateTime(query.submittedAt);
                      ...
                      return (
                        <tr key={`report-${query.queryId || `${query.appNo}-${queryMapIndex}`}-${queryMapIndex}`} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-4">
                            ...
                            {/* Display approval user for OTC/Deferral/Approved status */}
                            {(query.status === 'approved') && query.assignedTo && (
                              <div className="text-xs text-green-600 mt-1 font-medium">
                                Approved by: {query.assignedTo}
                              </div>
                            )}
                          </td>
                          <td className="px-4 py-4">
