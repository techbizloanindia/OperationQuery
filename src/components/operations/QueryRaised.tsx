...
const fetchQueries = async (): Promise<Query[]> => {
  try {
    ...
    const filteredQueries = result.data.filter((queryData: any) => {
      ...
      if (['request-approved', 'request-deferral', 'request-otc', 'approved', 'resolved', 'deferred', 'otc'].includes(queryData.status)) {
        return false;
      }
      ...
      const allQueriesResolved = queryData.queries?.every((q: any) => 
        ['request-approved', 'request-deferral', 'request-otc', 'approved', 'resolved', 'deferred', 'otc'].includes(q.status)
      );
  ...
  const individualQueries = React.useMemo(() => {
    ...
    queries.forEach(queryGroup => {
      queryGroup.queries.forEach((query, index) => {
        // Include all queries (pending, waiting for approval, etc.) except fully resolved ones
    ...
    <div className="h-full w-full bg-white overflow-hidden shadow-xl rounded-lg max-w-6xl mx-auto">
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                          ...
                          query.status === 'waiting for approval' ? 'bg-gray-100 text-gray-800' :
                          ...
                          {query.status === 'pending' ? 'Pending' :
                           query.status === 'waiting for approval' ? 'Request Pending' :
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-4 text-xs">
                      ...
                      <div className="text-gray-500">
                        ...
                        <span className="text-blue-600 font-medium">{query.tat || '24 hours'}</span>
                      </div>
                      <div className="text-gray-500">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            ...
            <div className="space-y-4">
              {/* Display operation user name for all actions */}
              <div>
                <label className="block text-sm font-bold text-black">Operation User</label>
                <input
                  type="text"
      ...
      {selectedQueryForChat && (
        <ModernChatInterface
          ...
          currentUser={{
            name: user?.name || 'Operations Team',
