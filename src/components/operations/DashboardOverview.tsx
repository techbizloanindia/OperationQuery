...
import { FaDownload, FaCircle, FaSync } from 'react-icons/fa';
...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    // Fetch real query statistics from API
    ...
    if (typeof window !== 'undefined') {
      import('@/lib/queryUpdateService').then(({ queryUpdateService }) => {
        ...
        const unsubscribe = queryUpdateService.subscribe('operations', (update) => {
          ...
          // Refresh all data when we receive updates
    ...
    // Fetch real-time application/sanctioned cases data
    const fetchApplicationStatistics = async () => {
      try {
        ...
        if (result.success) {
          setStatistics(prev => ({
            ...prev,
            totalSanctionedCases: result.data.total || 0
          }));
        }
      } catch (error) {
    ...
    // Set up real-time updates every 5 seconds (more responsive)
    ...
    }, 5000); // Reduced from 15 seconds to 5 seconds for better responsiveness
  ...
  const refreshAllData = async () => {
    ...
    try {
      ...
      if (applicationsResult.success) {
        updatedStats = {
          ...updatedStats,
          totalSanctionedCases: applicationsResult.data.total || 0
        };
      }
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-blue-700 hover:bg-blue-800 px-3 lg:px-4 py-2 rounded-lg flex items-center space-x-2 transition-colors text-sm lg:text-base"
            >
              <FaDownload />
        ...
        <div className="mt-4 flex items-center space-x-2 text-blue-100">
          ...
          <span className="text-xs lg:text-sm">
            {connectionStatus === 'connecting' ? 'Loading real-time data...' :
             connectionStatus === 'connected' ? 'Real-time data active (updates every 5s)' :
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-6">
        <div className="bg-white rounded-lg p-4 lg:p-6 shadow-sm border">
          <div className="text-center">
            ...
            <div className="text-gray-800 text-xs lg:text-sm font-bold">📋 TOTAL SANCTIONED CASES</div>
