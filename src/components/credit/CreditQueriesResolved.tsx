...
export default function CreditQueriesResolved() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = queryUpdateService.subscribe('credit', (update) => {
      ...
      // Check if this is a resolved query that's relevant to Credit team
      if ((update.action === 'resolved' || update.status === 'resolved') && 
          (update.markedForTeam === 'credit' || update.markedForTeam === 'both' || update.team === 'credit' || update.broadcast)) {
  ...
  const fetchResolvedQueries = async () => {
    try {
      ...
      if (result.success) {
        const creditResolvedQueries = result.data.filter((query: any) => 
          // More inclusive filter - include queries that might come from operations but are marked for credit
        ...
        console.log(`📊 CreditQueriesResolved: Found ${creditResolvedQueries.length} resolved queries`);
        setResolvedQueries(creditResolvedQueries);
      }
    } catch (error) {
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm text-gray-600 mb-3">
                      ...
                      <div className="flex items-center">
                        <User className="h-4 w-4 mr-2 text-gray-400" />
                        Resolved by: {query.resolvedBy || 'Credit Team'}
                      </div>
                    </div>
